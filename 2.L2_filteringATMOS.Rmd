...
knit: (function(inputFile, encoding) { 
      ...
      rmarkdown::render(inputFile,
                        encoding=encoding, 
                        output_file=file.path('/home/otto/data/atmos-flux-data/output/html/',
                        paste0(substr(inputFile,nchar(inputFile)-23,nchar(inputFile)-4), '_', Sys.Date(),'.html'))) })
...
# read fluxnet
input<-read.csv(paste('./processed/ATMOS_L1_fluxnet_2024-01-30.csv',sep=''))
input$DATE<-as.POSIXct(input$DATE,format="%Y-%m-%d %H:%M:%S", tz = "Etc/GMT+6")
fluxnet<-input

# read full_output
input<-read.csv(paste('./processed/ATMOS_L1_2024-01-30.csv',sep=''))
input$DATE<-as.POSIXct(input$DATE,format="%Y-%m-%d %H:%M:%S", tz = "Etc/GMT+6")
Data_QAQC<-input
```
...
raw.files <- list.files(path = path, pattern = "ATMOS484_.+.csv", recursive = FALSE)
...
# raw.data[raw.data$Timestamp > as.POSIXct('2023-03-31 16:00:00', tz = 'Etc/GMT+6'),(ncol(raw.data)-5):ncol(raw.data)]

data.ordered<-raw.data[order(raw.data$Timestamp, decreasing = FALSE ),]

data.ordered[1,1:5]
data.ordered[nrow(data.ordered),1:5]
raw.data[nrow(raw.data),1:5]
####
...
# Replacing -9999 and 99999 by NA
met[met == -9999] <- NA
met[met == 99999] <- NA
...
# as_datetime(paste0(year(met$DATE), '-01-01 00:00:00'), format = '%Y-%m-%d %H:%M:%S', tz = "Etc/GMT+6")
# as.POSIXct(as_datetime(paste0(year(met$DATE), '-01-01 00:00:00'), format = '%Y-%m-%d %H:%M:%S', tz = "Etc/GMT+6"), origin = "1970-01-01 00:00:00", tz = "Etc/GMT+6")

for (idx in na_idx){
  met$DOY_END[idx] <- difftime(met$DATE[idx], as.POSIXct(as_datetime(paste0(year(met$DATE[idx]), '-01-01 00:00:00'), format = '%Y-%m-%d %H:%M:%S', tz = "Etc/GMT+6"), origin = "1970-01-01 00:00:00", tz = "Etc/GMT+6"), units="days") + 1
}
...
```{r match met and flux time series, echo=FALSE, include=FALSE}
...
plot_ly(data = met, x = ~DATE, y = ~Tsoil_06, type = 'scatter', mode = 'lines') %>% 
...
plot_ly(data = met, x = ~DATE, y = ~RS, type = 'scatter', mode = 'lines') %>% 
  layout(title = 'SWC_mean') %>%  
  toWebGL()
...
output <- cbind(met[M_start:M_end,match(c('year','DOY_END','hour','TA_EP','RS'),names(met))], Data_QAQC[F_start:F_end,match(c('co2_flux','u.','wind_dir'),names(Data_QAQC))])
...
output <- output[c("year","DOY_END", "hour","co2_flux","RS","TA_EP","u.","wind_dir")]
...
output <- cbind(met[,c('year','DOY_END','hour','TA_EP','RS','RH_EP', 'VPD_hPa','Tsoil_mean')], 
...
output <- output[c("year", "DOY_END", "hour","co2_flux","ch4_flux","LE","H","RS","TA_EP","Tsoil_mean", "RH_EP","VPD_hPa","u.","wind_dir")]
